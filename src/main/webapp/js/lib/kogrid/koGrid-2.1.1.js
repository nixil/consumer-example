...
ko.bindingHandlers['koGrid'] = (function () {
    return {
        'init': function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            grid.config.data.subscribe(function () {
...
ko.bindingHandlers['kgToolbarLeft'] = (function () {
    return {
        'init': function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            var compile = function (html) {
                var content = $(html);
                for (var i = 0; i < content.length; i++)
                    ko.applyBindings(viewModel.$userViewModel, content[i]);
                $(element).html(content);
            };
            if (viewModel.leftToolbarTemplate) {
                compile(viewModel.leftToolbarTemplate);
            }
            return { controlsDescendantBindings: true };
        }
    };
...
ko.bindingHandlers['kgToolbarRight'] = (function () {
    return {
        'init': function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            var compile = function (html) {
                var content = $(html);
            ...
            if (viewModel.rightToolbarTemplate) {
                compile(viewModel.rightToolbarTemplate);
            }
            return { controlsDescendantBindings: true };
        }
    };
...
window.kg.Column = function (config, grid) {
    ...
    self.filterText.subscribe(function(data) {
        var searchText = '';
        $.each(grid.visibleColumns(), function(idx, item) {
            var val = item.filterText();
            if (val) {
                searchText += item.displayName() + ":" + val + ";";
            }
        });
        grid.config.filterOptions.filterText(searchText);
    });
    ...
    self.showFilterBox = (colDef.showFilterBox !== false && colDef.field !='\u2714'); //exclude selection column
...
window.kg.EventProvider = function (grid) {
    ...
    self.assignEvents = function () {
		...
		if(grid.config.jqueryUIDraggable){
			...
			grid.$headerScroller.on('mousedown', self.onHeaderMouseDown).on('dragstart', self.dragStart).on('dragover', self.dragOver).on('drop', self.onHeaderDrop);
    ...
    self.dragStart = function(evt) {
        //quirk dragdrop trick for firefox only
        // http://stackoverflow.com/questions/19907811/kogrid-column-reordering-is-not-working-in-mozilla-firefox
...
window.kg.RowFactory = function (grid) {
    ...
    self.renderedRange = grid.fixedViewPortHeight ? new window.kg.Range(0, grid.minRowsToRender() + EXCESS_ROWS) :
        grid.enablePaging ? new window.kg.Range(0, grid.pagingOptions.pageSize) : new window.kg.Range(0, 100000);

    self.changeRenderedRangeWithPageSize = function(newSize) {
        self.renderedRange = new window.kg.Range(0, newSize);
    };
    ...
    self.renderedChangeNoGroups = function() {
        var rowArr = [];
        var dataArr = grid.config.enablePaging ?
                            grid.sliceCurrentPageData(grid.filteredData).slice(self.renderedRange.topRow, self.renderedRange.calcMaxCanvasHeight) :
                            grid.filteredData.slice(self.renderedRange.topRow, self.renderedRange.bottomRow);
...
window.kg.Grid = function (options) {
    ...
    var defaults = {
            ...
            showToolbar: false,
            leftToolbarTemplate: undefined,
            rightToolbarTemplate: undefined,
            jqueryUITheme: false,
            ...
            //use fixed height or let height varies according to content
            ...
            enablePaging: false,
            pagingOptions: {
                pageSizes: ko.observableArray([10, 50, 100, 500]), //page Sizes
                pageSize: ko.observable(2), //Size of Paging data
    ...
    //save the unpaged data for sorting in the future
    self.origData = options.data;
    ...
    if (self.config.enablePaging) {
        //when paging is used
        //options.data = self.pagedData;  //this will cause the data source to be changed, should not be used.
        self.config.data = self.pagedData;
        self.config.useExternalSorting = true;
    }
    self.sliceCurrentPageData = function(data) {
        self.config.pagingOptions.totalServerItems(data().length);
        return data.slice((self.config.pagingOptions.currentPage() - 1) * self.config.pagingOptions.pageSize(),
                                self.config.pagingOptions.currentPage() * self.config.pagingOptions.pageSize());
    };
    ...
    self.showToolbar = self.config.showToolbar;
    self.leftToolbarTemplate = self.config.leftToolbarTemplate;
    self.rightToolbarTemplate = self.config.rightToolbarTemplate;
    if (self.config.enablePaging) {
        //an idle variable for triggering pagedData refresh
        self.pagedDataRefresher = ko.computed(function () {
            if (self.rowFactory)
                self.rowFactory.changeRenderedRangeWithPageSize(self.config.pagingOptions.pageSize());
            if (self.filterText().length > 0) {
                self.pagedData(self.filteredData.slice((self.config.pagingOptions.currentPage() - 1) * self.config.pagingOptions.pageSize(),
                                    self.config.pagingOptions.currentPage() * self.config.pagingOptions.pageSize()));
            }
            else {
                self.pagedData(self.origData.slice((self.config.pagingOptions.currentPage() - 1) * self.config.pagingOptions.pageSize(),
                                    self.config.pagingOptions.currentPage() * self.config.pagingOptions.pageSize()));
            }
        });

        self.config.sortInfo.subscribe(function (data) {
            //touch up the data source to trigger the sorting
            if (self.filterText().length > 0)
        ...
        self.config.pagingOptions.pageSize.subscribe(function() {
    ...
    self.calcMaxCanvasHeight = function() {
        ...
        ((self.config.enablePaging ? self.pagedData().length : self.filteredData().length)  * self.config.rowHeight);
    };
    self.elementDims = {
        scrollW: 0,
        scrollH: 0,
    ...
    self.configureColumnWidths = function() {
        ...
        if (asterisksArray.length > 0) {
            ...
            $.each(asterisksArray, function (i, col) {				
                ...
                if (col.index + 1 == numOfCols) {
                    ...
                    // are we overflowing?
                    if (self.config.fixedViewPortHeight &&
                        self.maxCanvasHt() > self.viewportDimHeight()) {
        ...
        if (percentArray.length > 0) {
            var gridWidth = self.config.displaySelectionCheckbox ?
                    self.rootDim.outerWidth() - self.elementDims.rowSelectedCellW : self.rootDim.outerWidth();
            // do the math
            $.each(percentArray, function (i, col) {
                var t = col.width;
                columns[col.index].width = Math.floor(gridWidth * (parseInt(t.slice(0, -1), 10) / 100));
                totalWidth += columns[col.index].width;
            });
        }

        /* adjust the last column to full width of the grid */
        if (totalWidth != self.rootDim.outerWidth() - 1) {
            var offset = 1;
            if (self.config.fixedViewPortHeight &&
                self.maxCanvasHt() > self.viewportDimHeight()) {
                //compensate for scrollbar
            ...
            columns[numOfCols - 1].width += self.rootDim.outerWidth() - offset - totalWidth;
        }
    ...
    self.adjustScrollLeft = function (scrollLeft) {
        ...
        if (self.$filterContainer) {
            self.$filterContainer.scrollLeft(scrollLeft);
        }
    };
	...
	self.viewportDimHeight = ko.computed(function () {
        return Math.max(0, self.rootDim.outerHeight() - self.topPanelHeight() - 24 - self.config.footerRowHeight - 2);
    });
    ...
    self.pageRange = function() {
        var min, max = 1;
        var maxPage = self.maxPages();
        var current = self.pagingOptions.currentPage();
        if (maxPage <= 5) {
            min = 1;
            max = maxPage;
        } else {
            min = current < 3 ? 1 : current - 2;
            max = min + 4 > maxPage ? maxPage : min + 4;
            if (max - min < 4) {
                min = min - (4 - (max - min));
            }
        }
...
window.kg.SearchProvider = function (grid) {
    ...
    self.evalFilter = function () {
        if (ko.isObservable(grid.config.sortInfo) && grid.config.sortInfo() && grid.config.sortInfo().column && grid.origData().length > 0)
            window.kg.sortService.Sort(grid.config.sortInfo(), grid.origData);
        if (searchConditions.length === 0) {
            //grid.filteredData(grid.sortedData.peek().filter(function(item) {
            grid.filteredData(grid.origData.peek().filter(function(item) {
            ...
            grid.filteredData(grid.origData.peek().filter(function(item) {
                ...
                for (var i = 0, len = searchConditions.length; i < len; i++) {
                    ...
                    if (condition.columnDef && condition.columnDef.cellFilter) field = condition.columnDef.cellFilter(field);
        ...
        $.each(columnFilters, function (i, filter) {
            ...
            if (args.length > 1) {
                ...
                if (columnName && columnValue) {
                    var columnDef = findColumnDef(columnName);
    ...
    var findColumnDef = function(columnName) {
        var columnDef;
        $.each(grid.columns(), function(i, v) {
            if (columnName == v.displayName()) {
                columnDef = v;
                return false;
            }
        });
...
window.kg.StyleProvider = function (grid) {
    ...
    grid.viewportStyle = ko.computed(function() {
        if (grid.config.fixedViewPortHeight)
...
window.kg.sortService = {
    ...
    sortData: function (data /*datasource*/, sortInfo) {
        ...
        unwrappedData.sort(function (itemA, itemB) {
            ...
            if (col.cellFilter) {
                propA = col.cellFilter(propA);
                propB = col.cellFilter(propB);
            }
...
window.kg.domUtilityService = {
    AssignGridContainers: function (rootEl, grid) {
        ...
        grid.$filterContainer = grid.$root.find(".kgFilter");
