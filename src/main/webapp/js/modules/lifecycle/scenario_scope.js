define(['jquery', 'knockout', 'modules/lifecycle/lifecycleServiceUtil', 'modules/common/user-validation', 'modules/common/dialog-utils', 'ojs/ojcore', 'ojs/ojknockout', 'ojs/ojchart'],
        function($, ko, util, user_validation, dialog) {
            ...
            function ScenarioScopeViewModel() {
                ...
                self.minVal   = ko.observable(1);
                self.maxVal   = ko.observable(100);
...
//--------------------------------------------------------------------------------------------------------------
                self.getScenarioList = function() {
                    ...
                    $.ajax({
						...
						console.log("user_validation.credential.userType() "+user_validation.credential.userType());
                            if (self.hasPrivilege('Life-Cycle-Edit')) {
                               $.each(data, function(idx, scenario) {
							scenario['isAdmin'] = true; 
							 self.scenarioList.push(scenario);
							});
							console.log("self.scenarioList for admin "+JSON.stringify(self.scenarioList()));
                            } else {
								...
								console.log("self.scenarioList for customer"+JSON.stringify(self.scenarioList()));
                            }
...
//------------------------------------------------------------------------------
    //-------------------------Begin AJAIN code---------------------------------
	            ...
	            if (data.isAdmin || data.status ==='Published')
		    {
                        ...
                        $.ajax({
                            ...
                            success: function(data, textStatus, jqXHR) {
                                ...
                                $.each(data, function(idx, item) {
                                    tblTrfMthdSer.push({name: item.RESULT_TEXT, items: [parseInt(item.RESULT_VALUE)]});
                                });
                    ...
                    self.pkgRunId.subscribe(function(data) {
                        ...
                        $.ajax({
                            ...
                            success: function(data, textStatus, jqXHR) {
                                ...
                                // Loop through the items
                                $.each(data, function(idx, item) {
                                    series2.push({name: item.RESULT_TEXT, items: [parseInt(item.RESULT_VALUE)]});
                                });
                    ...
                    self.pkgRunId.subscribe(function(data) {
                        ...
                        $.ajax({
                            ...
                            success: function(data, textStatus, jqXHR) {
                                ...
                                var series3 = [];
                                $.each(data, function(idx, item) {
                                    series3.push({name: item.RESULT_TEXT, items: [parseInt(item.RESULT_VALUE)]});
                                });
                                self.dataCompPer(series3);
                            }
                        });
                    });
                    //------------------Object Compression Projection--------------------
                    //self.migCompProj = ko.observableArray();
                    self.pkgRunId.subscribe(function(data) {
                        if (!data){
                            self.migCompProj([]);
                    ...
                    self.pkgRunId.subscribe(function(data) {
                        if (!data){
                            self.pDSizeofDataMig([]);
                            self.pGSizeofDataMig([]);                            
                            return;
                        }
                    ...
                    self.pkgRunId.subscribe(function(data) {
                        if (!data){
                            self.maxVal(null);
                            self.minVal(null);
                            self.pDDataThroughput([]);
                            self.pGDataThroughput([]);
                            return;
                        }
                        var urlForDataMigThroughput;
                        var urlForDataThroughputMinMaxVal;
                        urlForDataThroughputMinMaxVal = "rest/data/sql/dataThroughputMinMaxValue?scenario_id=" + self.scenarioId + "&pkg_run_id=" + data + "&engagement_id=" + self.engagement_Id;
                        ...
                        console.log("08A. The url is: " + urlForDataThroughputMinMaxVal);
                        $.ajax({
                            async   : true,
                            url     : urlForDataThroughputMinMaxVal,
                            datatype: "json",
                            contentType: "application/json; charset=utf-8",
                            success: function(data, textStatus, jqXHR) {
                                // Loop through the items
                                $.each(data, function(idx, item) {
                                    if (data && data.length > 0) {
                                        self.maxVal(parseInt(data[0].MAX_VAL));
                                        self.minVal(parseInt(data[0].MIN_VAL));
                                        console.log("The Min and Max Values = "+ self.minVal()+", "+self.maxVal());
                                    };
                                });
                            }
                        });
                        ...
                        $.ajax({
                            ...
                            success: function(data, textStatus, jqXHR) {
                                ...
                                // Loop through the items
                                $.each(data, function(idx, item) {
                                    dataSeries.push([parseInt(item.THROUGHPUT)]);
                        ...
                        //console.log("throughput series is: "+self.pDDataThroughput());
                    });
                    //---------Migration Job Status (very Large Tables not included)--------
                    //self.migJobStatusVLTNI = ko.observableArray();
                    self.pkgRunId.subscribe(function(data) {
                        if (!data){
                            self.migJobStatusVLTNI([]);
                            return;
                        }
                ...
                self.gridOmigJobStatusVLTNI = {data: self.migJobStatusVLTNI,
                    enablePaging: false,
                    showFilter: false,
                    columnDefs: [{field: 'job_description', displayName: 'Job Description', cellTemplate: '<span data-bind="text: getProperty($parent),attr: ($parent.entity.pkgdone && $parent.entity.pkgdone > 0) ? {} : {style:\'font-weight:bold\'}"></span>'},
                        {field: 'pkgdone', displayName: 'Pkg Done', width: 100},
                        {field: 'totalpkgs', displayName: 'Total Pkgs', width: 100},
                        {field: 'pct', displayName: 'Percentage', width: 200, cellTemplate: '<div class="progress progress-striped" data-bind="visible: ($parent.entity.pkgdone && $parent.entity.pkgdone > 0)" style="margin:5px; background-color:#f0ad4e">' +
                                    '<div class="progress-bar" role="progressbar"' +
                                    'data-bind="attr: {style: \'width:\' + getProperty($parent) + \'%\'}, text: getProperty($parent) + \'%\'"></div></div>'}
                    ]
                };
