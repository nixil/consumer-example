            ...
            function TamAdminViewModel() {
                ...
                self.beforeRoutingAction = function(context) {
                    ...
                    self.engagementGridOptions = {
                        ...
                        columnDefs: [{field: 'engagementId', displayName: 'Engagement Id', width: '8%'},
                            ...
                            {field: 'serviceId', displayName: 'Service Name', sortFn: util.sortFn, cellFormatter: function(data) {
                                    return self.getServiceName(data).toString();
                                }, width: '20%'},
                            {field: 'createDate', width: '14%', showFilterBox: false, cellTemplate: '<div class="kgActionText" data-bind="dateText: new Date($parent.entity[\'createDate\']),include: \'both\'"/>', displayName: 'Create Date'},
                            {field: null, displayName: 'Action', width: '8%', showFilterBox: false, cellTemplate: '<div class="kgAction"><button class="btn btn-default btn-sm" data-bind="click: function(){$parent.$grid.$userViewModel.editEngagement($parent.entity)}"> <i class="fa fa-pencil"></i>&nbsp;&nbsp;Edit</button> </div>'}
                        ]};
                    ...
                    self.scenarioGridOptions = {
                        ...
                        columnDefs: [{field: 'engagementId', displayName: 'Engagement Id', width: '10%'},
                            ...
                            {field: 'createDate', width: '18%', showFilterBox: false, cellTemplate: '<div class="kgActionText" data-bind="dateText: new Date($parent.entity[\'createDate\']),include: \'both\'"/>', displayName: 'Create Date'},
                            {field: null, displayName: 'Action', width: '30%', showFilterBox: false, cellTemplate: '<div class="kgAction">' +
                ...
                self.getSubmitScenarioRenderValue = function(engagementId) {
                    var engagement = ko.utils.arrayFirst(self.engagementList(), function(engagement) {
                        return (engagement.engagementId.toString() === engagementId.toString());
                    });
                    if (engagement && (engagement.status !== util.lifecycleStatus.new)) {
                        return true;
                    }
                    return false;
                };
                ...
                self.loadEngagementList = function() {
                    console.log("Getting Service Engagement List ...");
                    $.ajax({
                        type: 'POST',
                ...
                self.getServiceName = function(id) {
                    var service = ko.utils.arrayFirst(self.serviceList, function(object) {
                        return object.serviceId.toString() === id.toString();
                    });
...
//                    console.log("Editing Engagement...: " + JSON.stringify(row));
                ...
                self.getScenarioIdsForEngagement = function(engId) {
                    var scenarioList = ko.utils.arrayFilter(self.scenarioList(), function(scenario) {
                        if (scenario.engagementId.toString() === engId.toString())
                            return true;
                        return false;
                    });
                    var scenarioIds = "";
...
//                    console.log("Adding Scenario with data" + scenarioData);
                ...
                self.submitScenario = function(row) {
                    console.log("Submiting Scenario.. ");
                    ko.validation.disableValidation();
                    $('#modalPreloader').modal('show');
                    $.ajax({
                        type: 'POST',
