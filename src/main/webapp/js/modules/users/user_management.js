define(['jquery', 'knockout', 'date-util', 'moment-timezone', 'modules/common/dialog-utils', 'modules/common/user-validation', 'modules/common/httputils'],

    function ($, ko, dateUtil, moment, dialog, user_validation, httputils) {
        ...
        function UserManagementViewModel() {
            ...
            self.userID = ko.observable().extend({ required: true, pattern: {
                message: 'Only letters, numbers and underscore are allowed, system users are reserved',
                params: /(?!^setup$)(?!^root$)(?!^oracle$)(?!^itsp$)(^([a-zA-Z_][a-zA-Z0-9_]{0,30})$)/i
            }});
            self.userID.subscribe(function(data) {
                if (data) self.userID(data.toLowerCase());
            });
            ...
            self.gridOptions = { data: self.userList,
                                 ...
                                 columnDefs: [
                                     ...
                                     {field: 'userID', displayName: 'Action', width: '250px', showFilterBox: false, cellTemplate:'<div class="kgAction">' +
            ...
            self.selectUser = function (data) {
                for (var key in data) {
                    if (data.hasOwnProperty(key) && key != 'roles' && self.hasOwnProperty(key))
                        self[key](httputils.htmlDecode(data[key]));
                }
